...
router.findOne = function (req, res) {
    ...
    Income.find({ "_id": req.params.id }, function (err, income) {
        ...
        } else res.send(JSON.stringify(income, null, 5));
    });
};
...
router.addIncome = function (req, res) {
    //post record               post
    ...
    income.save(function (err) {
        if (err) res.json({ message: 'Income NOT Added!', errmsg: err });else res.json({ message: 'Income Successfully Added!', data: income });
    });
};

router.deleteIncome = function (req, res) {
    ...
    Income.findByIdAndRemove(req.params.id, function (err) {
        ...
        } else res.json({ message: 'Income Successfully Deleted!' });
    });
};
...
router.findUserTotalAmounts = function (req, res) {
    ...
    Income.find({ "email": req.params.email }, function (err, incomes) {
        ...
        else res.json([{ totalamounts: getTotalAmounts(incomes) }]);
    });
};

router.changeIncomeinfo = function (req, res) {
    ...
    Income.findById(req.params.id, function (err, income) {
        if (err) {
            ...
            income.save(function () {
                if (err) res.json({ Message: 'Income NOT Changed!', errmsg: err });else res.json({ Message: 'Income Successfully Changed!', data: income });
            });
        }
    });
};

router.findUserInByMessage = function (req, res) {
    ...
    Income.find({ "email": req.params.email, "message": { $regex: req.params.message, $options: 'i' } }, function (err, income) {
        if (income.length <= 0) res.json({ Message: 'Sorry! Cannot find income by this message!' });else res.send(JSON.stringify(income, null, 5));
    });
};

router.findUserMonthIncome = function (req, res) {
    //寻找每月支出额             get
    res.setHeader('Content-Type', 'application/json');
    //var keyword = {'date': {$regex:req.params.date, $options:'i'}};
    Income.find({ "email": req.params.email, "date": { $regex: req.params.date, $options: 'i' } }, function (err, incomes) {
        if (incomes.length <= 0) res.json({ Message: 'Sorry! Cannot find the incomes of this month!' });
        //res.send(err);
        else res.json([{ monthamounts: getTotalAmounts(incomes) }]);
    });
};
